<template>
	<view>
		<view class="container">
			...
			<view class="btn" @click="onOff=true"><i class="icon-add"></i>添加分类</view>
			<view class="cont">
				<view><span>分类名称</span><span>图片</span><span>操作</span></view>
				<view v-for="(item,index) in classifyArr"><span>{{item.name}}</span><span> <image :src="'http://123.57.187.184/zpw/img/'+item.imgurl"></image> </span><span><i class="icon-sc" @click="handleDelete(item.id,index)"></i></span></view>
			</view>
			<view class="model-add" v-if="onOff">
				<view class="add-top">
					<view class="add-title">添加分类</view>
					<view class="add-item"><span>分类名称:</span><span><input type="text" placeholder="请输入名称" v-model="title"></span></view>
					<view class="add-item"><span>分类图片:</span><span><i @click="handleUpFile" >{{bgImg==''?'点击上传':''}}<image v-if="bgImg!==''" :src="'http://123.57.187.184/zpw/img/'+bgImg+'.png'" mode=""></image></i><em>请上传400px-200px的图片</em></span></view>
	...
	export default{
		...
		methods:{
			handleDelete(v,i){
				let _this=this;
				uni.request({
					url:"http://123.57.187.184/zpw/os/delete/deleteClassify.php",
					method:"GET",
					data:{
						id:v
					},
			...
			handleAddAdvert(){
				...
				if(this.title!=='' || this.bgImg!==''){
					uni.request({
						url:"http://123.57.187.184/zpw/os/add/addClassify.php",
						method:"POST",
						data:{
							name:_this.title,
